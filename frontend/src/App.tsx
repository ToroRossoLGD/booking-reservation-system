...
export default function App() {
  ...
  function exploreCategory(label = "") { setQuery(label); closeHeaderMenus(); scrollToExplore(); }
  ...
  return <div className="app-shell">
    ...
    </nav><div className="nav-actions"><details className="mobile-menu" name="header-menu"><summary aria-label="Open navigation"><Icon name="menu"/></summary><div className="dropdown-panel mobile-panel"><strong>Explore</strong><button onClick={() => exploreCategory()}>All spaces</button>{categories.map((category) => <button key={category.label} onClick={() => exploreCategory(category.query)}>{category.label}</button>)}<a href="#how" onClick={closeHeaderMenus}>How it works</a><a href="#host" onClick={closeHeaderMenus}>List your space</a></div></details>{user ? <details className="profile-menu" name="header-menu"><summary className="profile-pill"><span>{user.email[0].toUpperCase()}</span><strong className="profile-name">{user.email.split("@")[0]}</strong><Icon name="chevron" size={14}/></summary><div className="dropdown-panel profile-panel"><div className="profile-identity"><strong>{user.email.split("@")[0]}</strong><small>{user.email}</small></div><button onClick={() => exploreCategory()}>Explore spaces</button><a href="#how" onClick={closeHeaderMenus}>How it works</a><a href="#host" onClick={closeHeaderMenus}>List or manage my space</a><button className="logout-item" onClick={logOut}><Icon name="logout" size={16}/>Log out</button></div></details> : <><button className="button ghost" onClick={() => setAuthMode("login")}>Log in</button><button className="button dark" onClick={() => setAuthMode("register")}>Sign up</button></>}</div></header>
    <main id="top">
