...
import type { AvailableResource, PopularVenue, Promotion, Quote, Resource, User, Venue } from "./types";

const categories = [
  { icon: "work", label: "Workspaces", query: "workspace", copy: "Desks, studios & meeting rooms" },
  { icon: "sport", label: "Sports", query: "sport", copy: "Courts, fields & training spaces" },
  { icon: "event", label: "Events", query: "event", copy: "Venues made for gathering" },
  { icon: "wellness", label: "Wellness", query: "wellness", copy: "Calm spaces for body & mind" },
];
...
export default function App() {
  ...
  const [page, setPage] = useState<"home" | "account" | "owner">(() => window.location.pathname.startsWith("/owner") ? "owner" : window.location.pathname.startsWith("/account") ? "account" : "home");
  const [toast, setToast] = useState("");
  const [searchDate, setSearchDate] = useState(""); const [searchTime, setSearchTime] = useState("10:00"); const [searchGuests, setSearchGuests] = useState(1); const [searchType, setSearchType] = useState("");
  const [availableMatches, setAvailableMatches] = useState<AvailableResource[] | null>(null); const [searchBusy, setSearchBusy] = useState(false);
  ...
  useEffect(() => { const closeMenus = (event: PointerEvent) => { if (!(event.target instanceof Element) || !event.target.closest("details")) document.querySelectorAll<HTMLDetailsElement>(".site-header details[open]").forEach((menu) => menu.removeAttribute("open")); }; document.addEventListener("pointerdown", closeMenus); return () => document.removeEventListener("pointerdown", closeMenus); }, []);
  useEffect(() => { const updatePage = () => setPage(window.location.pathname.startsWith("/owner") ? "owner" : window.location.pathname.startsWith("/account") ? "account" : "home"); window.addEventListener("popstate", updatePage); return () => window.removeEventListener("popstate", updatePage); }, []);
  const filtered = availableMatches === null ? venues.filter((v) => `${v.name} ${v.address} ${v.description ?? ""}`.toLowerCase().includes(query.toLowerCase())) : venues.filter((venue) => availableMatches.some((resource) => resource.venue_id === venue.id));
  function scrollToExplore() { document.getElementById("explore")?.scrollIntoView({ behavior: "smooth" }); }
  function closeHeaderMenus() { document.querySelectorAll<HTMLDetailsElement>(".site-header details[open]").forEach((menu) => menu.removeAttribute("open")); }
  function navigate(nextPage: "home" | "account" | "owner") { const path = nextPage === "home" ? "/" : `/${nextPage}`; window.history.pushState({}, "", path); setPage(nextPage); closeHeaderMenus(); window.scrollTo({ top: 0, behavior: "smooth" }); }
  function goHome(section?: string) { navigate("home"); if (section) window.setTimeout(() => document.getElementById(section)?.scrollIntoView({ behavior: "smooth" }), 0); }
  function exploreCategory(label = "") { setQuery(label); goHome("explore"); }
  async function performSearch() { if (!searchDate) { setAvailableMatches(null); scrollToExplore(); return; } const start = new Date(`${searchDate}T${searchTime}:00`); const end = new Date(start.getTime() + 60 * 60 * 1000); setSearchBusy(true); try { const result = await api.searchAvailable(start.toISOString(), end.toISOString(), searchGuests, searchType, query); setAvailableMatches(result.items); scrollToExplore(); } catch (error) { notify(error instanceof Error ? error.message : "Unable to search availability"); } finally { setSearchBusy(false); } }
  function logOut() { localStorage.removeItem("bookica_token"); setUser(null); }
  function notify(message: string) { setToast(message); window.setTimeout(() => setToast((current) => current === message ? "" : current), 2600); }
  async function saveFavorite(resourceId: number) { if (!user) { setAuthMode("login"); return; } try { await api.addFavorite(resourceId); notify("Saved to favorites."); } catch (error) { notify(error instanceof Error ? error.message : "Unable to save favorite"); } }
  ...
  return <div className="app-shell">
    <header className="site-header"><a className="brand" href="/" onClick={(event) => { event.preventDefault(); goHome(); }} aria-label="Bookica home"><span className="brand-mark">B</span><span>Bookica</span></a><nav>
      <details className="nav-dropdown" name="header-menu"><summary>Explore <Icon name="chevron" size={14}/></summary><div className="dropdown-panel explore-menu"><button onClick={() => exploreCategory()}>All spaces<small>Browse every available venue</small></button>{categories.map((category) => <button key={category.label} onClick={() => exploreCategory(category.query)}><Icon name={category.icon} size={18}/><span>{category.label}<small>{category.copy}</small></span></button>)}</div></details>
      <a href="/#how" onClick={(event) => { event.preventDefault(); goHome("how"); }}>How it works</a><a href="/#host" onClick={(event) => { event.preventDefault(); goHome("host"); }}>List your space</a>
    </nav><div className="nav-actions"><details className="mobile-menu" name="header-menu"><summary aria-label="Open navigation"><Icon name="menu"/></summary><div className="dropdown-panel mobile-panel"><strong>Explore</strong><button onClick={() => exploreCategory()}>All spaces</button>{categories.map((category) => <button key={category.label} onClick={() => exploreCategory(category.query)}>{category.label}</button>)}<button onClick={() => goHome("how")}>How it works</button><button onClick={() => goHome("host")}>List your space</button></div></details>{user ? <details className="profile-menu" name="header-menu"><summary className="profile-pill"><span>{user.email[0].toUpperCase()}</span><strong className="profile-name">{user.email.split("@")[0]}</strong><Icon name="chevron" size={14}/></summary><div className="dropdown-panel profile-panel"><div className="profile-identity"><strong>{user.email.split("@")[0]}</strong><small>{user.email}</small></div><button onClick={() => navigate("account")}>My reservations</button><button onClick={() => navigate("account")}>Favorites & notifications</button>{["owner", "admin"].includes(user.role) && <button onClick={() => navigate("owner")}>Owner workspace</button>}<button onClick={() => exploreCategory()}>Explore spaces</button><button className="logout-item" onClick={() => { logOut(); goHome(); }}><Icon name="logout" size={16}/>Log out</button></div></details> : <><button className="button ghost" onClick={() => setAuthMode("login")}>Log in</button><button className="button dark" onClick={() => setAuthMode("register")}>Sign up</button></>}</div></header>
    {page === "account" && user ? <AccountDashboard user={user} onExplore={() => goHome("explore")}/> : page === "owner" && user && ["owner", "admin"].includes(user.role) ? <OwnerDashboard onExplore={() => goHome("explore")}/> : <main id="top">
      <section className="hero"><div className="hero-glow one"/><div className="hero-glow two"/><div className="hero-copy"><div className="availability-pill"><span/> Spaces available near you</div><h1>The right space.<br/><em>Right when you need it.</em></h1><p>Discover and reserve remarkable places for work, play, and everything in between—all in a few effortless clicks.</p><div className="hero-search"><Icon name="search" size={23}/><input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && performSearch()} placeholder="Search spaces or locations" aria-label="Search venues"/><button onClick={performSearch} disabled={searchBusy}>{searchBusy ? "Searching…" : "Find a space"} <Icon name="arrow"/></button></div><div className="search-filters"><label>Date<input type="date" value={searchDate} min={new Date().toISOString().slice(0,10)} onChange={(event) => setSearchDate(event.target.value)}/></label><label>Time<input type="time" value={searchTime} onChange={(event) => setSearchTime(event.target.value)}/></label><label>Guests<input type="number" min={1} value={searchGuests} onChange={(event) => setSearchGuests(Number(event.target.value))}/></label><label>Type<select value={searchType} onChange={(event) => setSearchType(event.target.value)}><option value="">Any space</option><option value="workspace">Workspace</option><option value="sport">Sport</option><option value="event">Event</option><option value="wellness">Wellness</option></select></label></div><div className="trust-row"><span><Icon name="shield" size={17}/> Secure booking</span><span><Icon name="clock" size={17}/> Instant confirmation</span><span><Icon name="spark" size={17}/> Handpicked spaces</span></div></div>
      ...
      <section className="category-section"><div className="section-heading compact"><p className="eyebrow">Find your fit</p><h2>What are you making space for?</h2></div><div className="category-grid">{categories.map((c) => <button className="category-card" key={c.label} onClick={() => exploreCategory(c.query)}><span className="category-icon"><Icon name={c.icon} size={25}/></span><span><strong>{c.label}</strong><small>{c.copy}</small></span><Icon name="chevron" size={18}/></button>)}</div></section>
      <section className="explore-section" id="explore"><div className="section-heading"><div><p className="eyebrow">Explore nearby</p><h2>Spaces worth showing up for</h2></div><p>Flexible, trusted, and ready when you are.</p></div>
        {loading ? <div className="loading-grid">{[1,2,3].map((i)=><div className="venue-skeleton" key={i}/>)}</div> : apiOffline ? <div className="empty-state"><span><Icon name="spark" size={28}/></span><h3>We’re getting the spaces ready.</h3><p>Start the backend to load live venues. The frontend is connected and waiting.</p></div> : filtered.length === 0 ? <div className="empty-state"><span><Icon name="search" size={28}/></span><h3>{venues.length ? "No spaces match that search." : "The first spaces are coming soon."}</h3><p>{venues.length ? "Try another time, guest count, location, or category." : "Create a venue in the API and it will appear here automatically."}</p>{(query || availableMatches !== null) && <button className="text-button" onClick={()=>{setQuery("");setAvailableMatches(null);}}>Clear search</button>}</div> : <div className="venue-grid">{filtered.slice(0,6).map((venue,index)=><button className="venue-card" key={venue.id} onClick={()=>setSelectedVenue(venue)}><span className={`venue-visual visual-${index%3}`}><span className="venue-type">Bookable venue</span><span className="visual-monogram">{venue.name.slice(0,2).toUpperCase()}</span></span><span className="venue-body"><span><strong className="venue-heading">{venue.name}</strong><span className="venue-location"><Icon name="pin" size={15}/>{venue.address}</span></span><span className="card-arrow"><Icon name="arrow"/></span><span className="venue-description">{venue.description || "A flexible space ready for your next plan."}</span><span className="venue-meta"><span><Icon name="clock" size={16}/> From {venue.minimum_booking_duration_minutes} min</span><span>Free cancellation · {venue.free_cancellation_hours}h</span></span></span></button>)}</div>}
      </section>
