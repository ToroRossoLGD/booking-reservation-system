...
function PropertyCard({ property: p }: { property: PropertyListing }) {
  ...
  return <article className="ph-card">
    <div className={`ph-card-art ph-scene-${p.offer_type === "short_stay" ? "sea" : "city"}`}>
      ...
      <span className="ph-art-label">ILUSTRACIJA · FOTOGRAFIJE USKORO</span>
    </div>
    <div className="ph-card-body">
      <p className="ph-location">{p.city}</p><h3>{p.title}</h3>
      ...
      <div className="ph-card-bottom"><p><strong>{propertyPrice(p)}</strong> / {priceUnits[p.offer_type]}</p>
        <button aria-expanded={expanded} aria-controls={`property-${p.id}`} onClick={() => setExpanded(!expanded)} aria-label={`Detalji: ${p.title}`}>{expanded ? "−" : "↗"}</button>
      </div>
      {expanded && <div id={`property-${p.id}`} className="ph-detail">
        <p className="ph-description">{p.description}</p>
        {p.offer_type === "short_stay" && <p>Dostupnost i konačnu cenu dogovorite sa vlasnikom. Online rezervacija ovog smeštaja još nije dostupna.</p>}
...
export default function PropertyHome() {
  ...
  return <div className="property-home">
    <header className="ph-header">
      ...
      <nav aria-label="Glavna navigacija"><a href="#ponuda">Pronađi nekretninu</a><a href="#destinacije">Destinacije</a><a href="/account">Moj nalog</a></nav>
    ...
    <main>
      ...
      <section className="ph-listings" id="ponuda" aria-busy={loading}>
        ...
        {loading ? <p role="status">Učitavanje oglasa…</p> : error ? <div className="ph-empty" role="alert"><h3>Ponuda trenutno nije dostupna.</h3><p>Pokušaj ponovo za nekoliko trenutaka.</p><button className="ph-outline" onClick={() => search(query)}>Pokušaj ponovo</button></div> : <>
          ...
          {page.items.length === 0 && <div className="ph-empty"><h3>Nema oglasa za ovaj izbor.</h3><p>Promeni pretragu ili objavi prvu nekretninu.</p><button className="ph-outline" onClick={() => search("", "")}>Prikaži sve oglase</button></div>}
