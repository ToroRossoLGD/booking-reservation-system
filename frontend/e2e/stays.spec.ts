...
test(`guest quotes, reserves, views and cancels from ${detailPage ? "listing page" : "catalog"}`, async ({ page }, testInfo) => {
  ...
  await page.route("**/api/properties/4/stay-quote", route => {
    ...
    return route.fulfill({ json: { nights: 3, nightly_rate_cents: 6500, total_cents: 19500, currency: "EUR", check_in_time: "15:00", check_out_time: "10:30", timezone: "Europe/Belgrade", payment_method: "pay_on_arrival" } });
  });
  await page.route("**/api/properties/4/stays", route => {
    expect(route.request().postDataJSON()).toMatchObject({ expected_total_cents: 19500, expected_currency: "EUR", expected_check_in_time: "15:00", expected_check_out_time: "10:30", expected_timezone: "Europe/Belgrade", check_in: arrival, check_out: departure, guests: 2 });
    confirmed = true;
  ...
  await expect(page.getByText(/Prijava od 15:00/)).toContainText("Odjava do 10:30 (Europe/Belgrade)");
